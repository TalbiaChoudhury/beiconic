...
const VisionCarousel = () => {
  ...
  // Removed: autoscrollIntervalRef = useRef(null); // Removed autoscroll ref
  ...
  // Removed: Function to reset the autoscroll timer
  // const resetAutoscroll = useCallback(() => { ... }, [visionSlides.length]);

  const handlePrev = () => {
    // Removed: resetAutoscroll(); // Removed call to reset autoscroll
  ...
  // Removed: Autoscroll effect // Removed the entire autoscroll useEffect block
  // useEffect(() => {
  //   resetAutoscroll();
  //   return () => {
  ...
  useEffect(() => {
    const handleHighlightUserForm = () => {
      console.log('VisionCarousel: highlightVisionUserForm event received!');
    ...
    const handleHighlightTrainerForm = () => {
      console.log('VisionCarousel: highlightVisionTrainerForm event received!');
            ...
            <div key={slide.id} className={styles.slide}>
              ...
              <div className={styles.textContainer}>
                ...
                <div className={styles.text}>
                  {/* Conditionally render forms only for slide 10 (index 9) */}
                  {index === 9 ? (
                    <>
                      {slide.content}
                      <div className={styles.formsContainer}>
                        ...
                        <div className={styles.formSection}>
                          {/* NEW: Wrapper for inline title and input */}
                          <div className={styles.formRow}>
                            <h3 className={styles.formTitle}>Users</h3> {/* Changed from "For Users (Clients)" */}
                            <input
                              type="email"
                        ...
                        <div className={styles.formSection}>
                          ...
                          <button type="submit" className={`${styles.signupButton} ${styles.trainersButton}`} disabled={trainerLoading} onClick={handleTrainerSubmit}>
                            {trainerLoading ? 'Applying...' : 'Apply Now'}
                          </button>
                          {trainerMessage && <p className={styles.message}>{trainerMessage}</p>}
                        </div>
                      </div>
                    </>
                  ) : (
                    // Render original content for other slides
                    slide.content
                  )}
                </div>
              </div>
